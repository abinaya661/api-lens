...
**Platform adapters** (`lib/platforms/adapters/*.ts`) implement `validateKey` and `fetchUsage`. Registry at `lib/platforms/registry.ts`. Sync engine at `lib/platforms/sync-engine.ts` orchestrates cron-based usage sync, budget checks, and key health tracking.
...
Supabase Postgres with RLS on all tables. Migrations in `supabase/migrations/` (001-011). Auth trigger auto-creates profile, company, and trial subscription on signup. Core ownership chain: `auth.users` -> `companies` -> `projects`/`api_keys`/`budgets`.
