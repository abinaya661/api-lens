...
function getHealthConfig(key: ApiKey) {
  if (!key.is_active) return { icon: ShieldX, color: 'text-zinc-400', bg: 'bg-zinc-500/10', label: 'Revoked' };
  if (!key.is_valid || key.consecutive_failures > 0) return { icon: ShieldX, color: 'text-red-400', bg: 'bg-red-500/10', label: 'Error' };
  return { icon: Shield, color: 'text-green-400', bg: 'bg-green-500/10', label: 'Healthy' };
}
