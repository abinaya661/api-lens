...
export default function ProjectsPage() {
  ...
  if (isLoading) {
    return (
      <div className="animate-fade-in">
        <PageHeader title="Projects" description="Organize API keys by project for cost attribution." />
  ...
  const archivedProjects = allProjects.filter((p) => !p.is_active);
  ...
  function handleSave() {
    ...
    if (editProject) {
      updateMutation.mutate(
        { id: editProject.id, name: formName, description: formDesc || null },
  ...
  function handleArchive(id: string, currentlyActive: boolean) {
    updateMutation.mutate({ id, is_active: !currentlyActive });
    setMenuOpen(null);
  }
    ...
    <div className="animate-fade-in">
      ...
      {allProjects.length === 0 ? (
        <EmptyState
          ...
          description="Create your first project to organize API keys and track costs per project."
          action={
            <button
              onClick={openCreate}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg bg-brand-600 text-white hover:bg-brand-700 transition-colors"
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">
            {activeProjects.map((project) => (
              <div key={project.id} className="glass-card p-5 flex flex-col gap-4 relative group">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-brand-600/10 text-brand-400" style={{ backgroundColor: `${project.color}15`, color: project.color }}>
                      <FolderOpen className="w-5 h-5" />
                    </div>
                    <div>
                      <h3 className="text-sm font-semibold text-zinc-100">{project.name}</h3>
                      <p className="text-xs text-zinc-500 mt-0.5">{timeAgo(project.created_at)}</p>
                    </div>
                  </div>
                  <div className="relative">
                    ...
                    {menuOpen === project.id && (
                      <div className="absolute right-0 top-8 z-50 w-40 bg-zinc-900 border border-zinc-800 rounded-lg shadow-xl py-1 animate-fade-in">
                        <button onClick={() => openEdit(project)} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-800 transition-colors">
                          <Pencil className="w-3.5 h-3.5" /> Edit
                        </button>
                        <button onClick={() => handleArchive(project.id, true)} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-800 transition-colors">
                          <Archive className="w-3.5 h-3.5" /> Archive
                        </button>
                        <button onClick={() => handleDelete(project.id)} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-400 hover:bg-zinc-800 transition-colors">
          ...
          {archivedProjects.length > 0 && (
            <div>
              <h2 className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-3">Archived ({archivedProjects.length})</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {archivedProjects.map((project) => (
                  <div key={project.id} className="glass-card p-5 opacity-50 hover:opacity-80 transition-opacity relative group">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => handleArchive(project.id, false)} className="text-xs text-brand-400 hover:text-brand-300 opacity-0 group-hover:opacity-100 transition-all">
                        Restore
                      </button>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="glass-card w-full max-w-md p-6 mx-4">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-white">{editProject ? 'Edit Project' : 'Create Project'}</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="e.g. Production Backend"
              ...
              <div>
                ...
                <textarea value={formDesc} onChange={(e) => setFormDesc(e.target.value)} placeholder="What is this project used for?" rows={3}
