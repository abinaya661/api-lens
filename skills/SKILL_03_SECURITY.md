...
If API Lens stores third-party API keys and has a breach, the blast
radius is enormous — users' OpenAI, AWS, and Anthropic accounts are exposed.
...
const IV_BYTES   = 12  // 96-bit IV — GCM standard recommends exactly 12 bytes.
                       // 16 bytes requires an extra GHASH computation.
...
export function getKeyHint(plaintext: string): string {
  ...
  // Stored as plain text separately. The last 4 chars alone cannot
  // reconstruct or guess the key. This avoids a decryption operation
...
export function maskForDisplay(provider: string, hint: string): string {
  const prefixes: Record<string, string> = {
    ...
    aws: 'AKIA...', azure: 'az-...', mistral: 'ms-...', cohere: 'co-...', custom: '...'
  }
...
## 10 Security Rules — with reasoning
...
a platform API call. List queries, dashboard queries, table renders —
none need the encrypted value. If it never appears unnecessarily in
JavaScript memory, it cannot accidentally leak into a log or response.
...
const { data } = await supabase
  .from('api_keys')
  .select('id,provider,nickname,key_hint,is_active,last_used')
...
},
{
  ...
  value: 'nosniff'
  // Prevents MIME-type sniffing — a text file cannot be executed as JavaScript.
},
{
  key: 'X-Frame-Options',
  value: 'DENY'
...
Rule 5: RLS on every table — see SKILL_02 for full implementation.
Rule 6: Webhook signature verified — see SKILL_05 for Razorpay implementation.
Rule 7: Env vars validated at startup — see below.
Rule 8: Never log plaintext keys anywhere in any context.
Rule 9: Stripe/Razorpay card data never touches our servers — use hosted checkout.
Rule 10: Error messages to users are clean English. Never expose internal details.
